...
func TestConnector_Type(t *testing.T) {
	...
	assert.Equal(t, "google-drive", conn.Type())
}
...
func TestConnector_Capabilities(t *testing.T) {
	...
	assert.True(t, caps.SupportsIncremental, "Drive supports incremental sync via Changes API")
	assert.False(t, caps.SupportsWatch, "Drive does not support watch in CLI")
	assert.True(t, caps.SupportsHierarchy, "Drive has folder hierarchy")
	assert.False(t, caps.SupportsBinary, "Drive exports text content")
	assert.True(t, caps.RequiresAuth, "Drive requires OAuth")
	assert.True(t, caps.SupportsValidation, "Drive supports validation")
	assert.True(t, caps.SupportsCursorReturn, "Drive returns cursor on sync complete")
	assert.True(t, caps.SupportsPartialSync, "Drive supports partial sync")
	assert.True(t, caps.SupportsRateLimiting, "Drive connector handles rate limiting")
	assert.True(t, caps.SupportsPagination, "Drive uses pagination")
}
...
func TestConnector_IncrementalSync_WhenClosed(t *testing.T) {
	...
	cursor := NewCursor()
	cursor.StartPageToken = "some-token"
...
func TestBuildFolderQuery(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "empty folder IDs",
			folderIDs: []string{},
			expected:  "",
		},
		{
			name:      "single folder ID",
			folderIDs: []string{"folder-123"},
			expected:  "'folder-123' in parents",
		},
		{
			name:      "multiple folder IDs",
			folderIDs: []string{"folder-1", "folder-2", "folder-3"},
			expected:  "'folder-1' in parents or 'folder-2' in parents or 'folder-3' in parents",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := buildFolderQuery(tt.folderIDs)
