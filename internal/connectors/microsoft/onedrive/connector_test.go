...
func TestConnector_Capabilities(t *testing.T) {
	...
	assert.True(t, caps.SupportsIncremental, "OneDrive supports incremental sync via delta queries")
	assert.False(t, caps.SupportsWatch, "OneDrive does not support watch in CLI")
	assert.True(t, caps.SupportsHierarchy, "OneDrive has folder hierarchy")
	assert.False(t, caps.SupportsBinary, "OneDrive returns text content")
	assert.True(t, caps.RequiresAuth, "OneDrive requires OAuth")
	assert.True(t, caps.SupportsValidation, "OneDrive supports validation")
	assert.True(t, caps.SupportsCursorReturn, "OneDrive returns cursor on sync complete")
	assert.True(t, caps.SupportsPartialSync, "OneDrive supports partial sync")
	assert.True(t, caps.SupportsRateLimiting, "OneDrive connector handles rate limiting")
	assert.True(t, caps.SupportsPagination, "OneDrive uses pagination")
}
...
func TestConnector_IncrementalSync_WhenClosed(t *testing.T) {
	...
	cursor := NewCursor()
	cursor.SetDeltaLink("https://graph.microsoft.com/v1.0/me/drive/root/delta?token=abc")
...
func TestConnector_buildDeltaURL(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			folderIDs: []string{"folder-abc-123"},
			contains: []string{
				"/me/drive/items/folder-abc-123/delta",
				"$top=100",
			},
		},
	}
...
func TestConnector_buildDeltaURL_CustomMaxResults(t *testing.T) {
	cfg := DefaultConfig()
	cfg.MaxResults = 50
	conn := New("source-123", cfg, nil)
